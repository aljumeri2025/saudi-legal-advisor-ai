...
export const HistorySidebar: React.FC<HistorySidebarProps> = ({ 
  ...
  const filteredSessions = sessions.filter(s => 
    s.title.includes(searchTerm) || s.preview.includes(searchTerm)
  );
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pr-9 pl-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-saudi-green/20 focus:border-saudi-green outline-none transition-all"
              value={searchTerm}
          ...
          {filteredSessions.length === 0 ? (
            ...
            filteredSessions.map(session => (
              <div
                ...
                className={`group relative w-full text-right p-4 rounded-2xl border cursor-pointer transition-all duration-200 ${
              ...
              >
                ...
                <p className="text-xs text-gray-500 truncate pl-6 opacity-80 font-medium">
                  {session.preview}
                </p>
                ...
                <button 
                  ...
                  className="absolute left-3 top-1/2 -translate-y-1/2 p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all z-10 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 focus:opacity-100"
                  title="حذف المحادثة"
        ...
        <div className="p-4 border-t border-gray-100 text-center bg-gray-50/30">
            <p className="text-[10px] text-gray-400">يتم حفظ المحادثات محلياً على جهازك</p>
        </div>
      </div>
    </>
  );
