      ...
      try {
        ...
        const sessionPromise = ai.live.connect({
          ...
          config: {
            ...
            systemInstruction:
              'أنت مستشار قانوني سعودي (رجل). تحدث بنبرة صوتية رزينة ومهنية. استخدم اللهجة السعودية البيضاء والمصطلحات القانونية الدقيقة عند الحاجة. هدفك هو مساعدة المستخدم صوتياً.',
          },
        });
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md px-6 relative">
        ...
        <div className="relative">
          {status === 'connected' && (
            <>
              <div
                className="absolute inset-0 bg-saudi-green rounded-full opacity-20 animate-ping"
                style={{ transform: `scale(${1 + volumeLevel})` }}
              ></div>
              <div
                className="absolute inset-0 bg-saudi-gold rounded-full opacity-10 animate-pulse delay-75"
                style={{ transform: `scale(${1.2 + volumeLevel})` }}
              ></div>
            </>
          )}
          ...
          <div
            className={`relative w-32 h-32 rounded-full flex items-center justify-center transition-all duration-500 ${
              status === 'connected'
                ? 'bg-gradient-to-br from-saudi-green to-saudi-dark shadow-2xl shadow-saudi-green/50'
                : 'bg-gray-700'
            }`}
          >
            {status === 'connecting' ? (
              ...
              <Volume2
                ...
                className={`text-white transition-opacity duration-200 ${
                  volumeLevel > 0.1 ? 'opacity-100' : 'opacity-70'
                }`}
              />
            )}
          </div>
